...
class Store(object):
    ...
    def get(self, cls, key, for_update=False):
        ...
        select = Select(cls_info.columns, where,
                        default_tables=cls_info.table, limit=1, for_update=for_update)
...
class ResultSet(object):
    ...
    def config(self, distinct=None, offset=None, limit=None, for_update=None):
    ...
    def _get_select(self):
        ...
        return Select(columns, self._where, self._tables, default_tables,
                      ...
                      having=self._having, for_update=self._for_update)
    ...
    def is_empty(self):
        ...
        # Should we really strip FOR UPDATE clause or should we raise a
        # FeatureError?
        subselect.for_update = False
    ...
    def for_update(self):
        """Acquire an exclusive row-level lock on the rows in the ResultSet
        """
        if self._group_by is not Undef:
            raise FeatureError("SELECT ... FOR UPDATE isn't supported with a "
                               " GROUP BY clause ")
    ...
    def _aggregate(self, aggregate_func, expr, column=None):
        if self._group_by is not Undef:
            raise FeatureError("Single aggregates aren't supported after a "
                               " GROUP BY clause ")
        if self._for_update:
            raise FeatureError("Single aggregates aren't supported with"
                               " FOR UPDATE clause ")
        columns, default_tables = self._find_spec.get_columns_and_tables()
    ...
    def union(self, other, all=False):
        ...
        if isinstance(other, EmptyResultSet):
            return self
        if self._for_update or other._for_update:
            raise FeatureError("SELECT FOR UPDATE is not allowed with UNION")
    ...
    def difference(self, other, all=False):
        ...
        if isinstance(other, EmptyResultSet):
            return self
        if self._for_update or other._for_update:
            raise FeatureError("SELECT FOR UPDATE is not allowed with EXCEPT")
    ...
    def intersection(self, other, all=False):
        ...
        if isinstance(other, EmptyResultSet):
            return other
        if self._for_update or other._for_update:
            raise FeatureError("SELECT FOR UPDATE is not allowed with "
                               "INTERSECT")
